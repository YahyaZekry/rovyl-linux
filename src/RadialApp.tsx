...
export default function RadialApp() {
  ...
  /**
   * Wayland cursor truth: this document receives the real forwarded motion while the wheel is
   * open (`getCursorScreenPoint` answers 0,0 in the main process on Wayland), and main
   * translates client coords with the BLOCK rect origin to feed the helper's `POS`. Top-level
   * effect — hooks must never nest.
   */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    /**
     * A click the helper swallowed outside the wheel: click away closes it.
     */
